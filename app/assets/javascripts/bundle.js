					...
					menuts[i].addEventListener("click",
						function(e) {
							...
							if (t) {
								...
								e.target.toggleAttribute("menu-open");
								/* calculate if the menu can't fit in the space below */
								V.menu.toporbottom(t, e.target);
								V.menu.leftorright(t, e.target);
							}
...
function mainToggler() {
	tog = document.getElementById("menu-tog");
...
document.addEventListener("turbolinks:load", function() { V.init() });
document.addEventListener("turbolinks:load", function() { mainToggler() });
document.addEventListener("turbolinks:load", function() { console.log("done") })